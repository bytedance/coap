...
def main(args):
    ...
    def transform_images(examples):
        idx_name = "image"
        if idx_name not in examples:
            idx_name = "img"
        images = [augmentations(image.convert("RGB")) for image in examples[idx_name]]
        return {"input": images}
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if accelerator.is_main_process:
                    show_memory_usage(model, optimizer)
        ...
        if accelerator.is_main_process:
            if args.logger == "tensorboard":
                ...
                tracker.add_scalar("CUDA/Total_Memory", total_memory, epoch)
                tracker.add_scalar("CUDA/Reserved_Memory", reserved_memory, epoch)
                tracker.add_scalar("CUDA/Allocated_Memory", allocated_memory, epoch)
                tracker.add_scalar("CUDA/Free_Memory", free_memory, epoch)
                tracker.add_scalar(
                    "CUDA/Max_Allocated_Memory", max_allocated_memory, epoch
                )
                tracker.add_scalar(
                    "CUDA/Max_Reserved_Memory", max_reserved_memory, epoch
                )
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                generator = torch.Generator(device=pipeline.device).manual_seed(3407)
