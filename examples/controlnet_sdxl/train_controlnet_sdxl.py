...
from coap_torch import CoapAdamW, CoapAdafactor, show_memory_usage
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    model_description = f"""
# controlnet_sdxl-{repo_id}

These are controlnet_sdxl weights trained on {base_model} with new type of conditioning.
{img_str}
"""
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to pretrained controlnet_sdxl model or model identifier from huggingface.co/models."
        " If not specified controlnet_sdxl weights are initialized from unet.",
    )
    parser.add_argument(
        "--variant",
        type=str,
    ...
    parser.add_argument(
        ...
        help="The column of the dataset containing the controlnet_sdxl conditioning image.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of paths to the controlnet_sdxl conditioning image be evaluated every `--validation_steps`"
    ...
    if args.resolution % 8 != 0:
        raise ValueError(
            "`--resolution` must be divisible by 8 for consistently sized encoded images between the VAE and the controlnet_sdxl encoder."
        )
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        logger.info("Loading existing controlnet_sdxl weights")
        controlnet = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet_sdxl weights from unet")
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = ControlNetModel.from_pretrained(
                    input_dir, subfolder="controlnet_sdxl"
                )
    ...
    params_to_opt = controlnet.parameters()

    if optimizer_name == "coap_adamw":
        optimizer = CoapAdamW(
            ...
            rank_ratio_matrix=args.rank_ratio_matrix,
            rank_ratio_cnn=args.rank_ratio_cnn,
        )
    elif optimizer_name == "coap_adafactor":
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    show_memory_usage(unet, optimizer)
    ...
    if accelerator.is_main_process:
        ...
        # Setting `vae`, `unet`, and `controlnet_sdxl` to None to load automatically from `args.output_dir`.
        image_logs = None
